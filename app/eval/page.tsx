...
export default function EvalPage(){
  ...
  useEffect(()=>{if(!runId)return;const t=setInterval(()=>void refresh(runId),2500);return()=>clearInterval(t)},[refresh,runId]);

  async function start(){if(active)return;setBusy(true);setError("");try{const d=await request("POST");const id=String(d.run_id);setRunId(id);setRun(d.run);window.setTimeout(()=>void refresh(id),1000)}catch(e){setError(e instanceof Error?e.message:"Eval konnte nicht gestartet werden")}finally{setBusy(false)}}
