...
function bookCover(category:string,facts:Array<{label:string;value:string}>,aiUrl:string|null){if(category!=="Buch")return aiUrl;const isbnFact=facts.find(f=>f.label.toLowerCase().includes("isbn"));const isbn=isbnFact?.value.replace(/[^0-9Xx]/g,"");if(isbn&&isbn.length>=10)return `https://covers.openlibrary.org/b/isbn/${isbn}-L.jpg?default=false`;return aiUrl}
...
- image_url nur als direkte HTTPS-Bild-URL, wenn belastbar; bei Büchern wird serverseitig zusätzlich über die ISBN ein Cover aufgelöst.
