...
export default function IdeaDetailPage(){
  ...
  return <main className="mx-auto min-h-[100dvh] w-full max-w-[430px] bg-[#fbfaf7] text-[#111]">
    ...
    <div className="relative mx-4 flex h-[220px] items-center justify-center overflow-hidden rounded-[24px] bg-[#e5eadf]">{processing?<KipuThinking label="Kipu macht im Hintergrund weiter"/>:image&&!imgFailed?<img src={image} alt={title} onError={()=>setImgFailed(true)} decoding="async" className={`h-full w-full ${fit==="contain"?"object-contain p-4":"object-cover"}`} referrerPolicy="no-referrer"/>:<span className="text-[58px]">{emoji}</span>}{!processing&&!failed&&<Link prefetch={false} href={`/ideas/${id}/edit?focus=image`} aria-label="Bild bearbeiten" className="absolute bottom-3 right-3 flex h-9 w-9 items-center justify-center rounded-full bg-white/95 shadow"><Pencil className="h-4 w-4"/></Link>}</div>

    <section className="px-4 pb-8 pt-5">
      {processing&&<div className="mb-4 rounded-[16px] bg-[#eef5e5] px-4 py-3"><div className="flex items-start gap-3"><Sparkles className="mt-0.5 h-5 w-5 shrink-0 text-[#65942c]"/><div className="min-w-0 flex-1"><p className="text-[12px] font-semibold text-[#65942c]">✓ Gespeichert – du kannst weitergehen.</p><p className="mt-1 text-[10px] leading-4 text-black/48">Kipu macht im Hintergrund weiter. Du kannst die App weiter benutzen oder schliessen.</p></div></div><Link href="/" className="mt-3 inline-flex rounded-full bg-white px-3 py-2 text-[10px] font-semibold text-black/65 shadow-[0_2px_8px_rgba(0,0,0,.04)]">Zur Startseite</Link></div>}
