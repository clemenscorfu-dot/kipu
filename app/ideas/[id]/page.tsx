...
import {ArrowLeft,Check,ChevronDown,Copy,ExternalLink,Info,LoaderCircle,Map,MapPin,Pencil,Share2,Sparkles,Trash2} from "lucide-react";
...
function KipuThinking(){return <div className="kipu-thinking"><span className="kipu-orbit kipu-orbit-one"/><span className="kipu-orbit kipu-orbit-two"/><span className="kipu-spark kipu-spark-one">✦</span><span className="kipu-spark kipu-spark-two">✧</span><div className="kipu-bot"><span className="kipu-antenna"/><span className="kipu-ear kipu-ear-left"/><span className="kipu-ear kipu-ear-right"/><span className="kipu-face"><i/><i/></span></div><span className="kipu-thinking-label">Kipu denkt nach</span></div>}
...
export default function IdeaDetailPage(){
  ...
  useEffect(()=>{
    if(!uuid.test(id)){setError("Erinnerung nicht gefunden.");setLoading(false);return}
    ...
    async function load(){try{
      ...
      if(!data){if(active){setStored(null);setError("Erinnerung nicht gefunden.");setLoading(false)}return}
    ...
    }catch(e){if(active&&!preview)setError(e instanceof Error?e.message:"Erinnerung konnte nicht geladen werden.")}finally{if(active)setLoading(false)}}
  ...
  async function remove(){if(!uuid.test(id)||!confirm("Diese Erinnerung wirklich löschen?"))return;setDeleting(true);const{error}=await getSupabaseBrowserClient().from("ideas").delete().eq("id",id);if(error){alert("Löschen fehlgeschlagen: "+error.message);setDeleting(false);return}router.replace("/ideas")}
