...
export default function IdeasPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        for (const row of (imgs.data ?? []) as Array<{ id: string; image_url: string | null }>) {
          if (row.image_url) imageById.set(row.id, row.image_url);
        }

        const next = ((i.data ?? []) as unknown as IdeaRow[])
          .filter((x) => x.processing_status !== "merged")
          .map((x) => ({ ...x, image_url: imageById.get(x.id) ?? null }));
        ...
        if (needs && !curatorStarted.current) {
          ...
          const session = (await s.auth.getSession()).data.session;
          if (session?.access_token) {
            void fetch("/api/categories/backfill", { method: "POST", headers: { Authorization: `Bearer ${session.access_token}` } })
              .then((r) => {
                if (!r.ok) throw new Error();
                if (active) timer = setTimeout(load, 700);
              })
              .catch(() => active && setError("Kipu konnte die Kategorien noch nicht aufbauen."))
              .finally(() => active && setOrganizing(false));
          }
        } else if (next.some((x) => ["pending", "processing"].includes(x.processing_status ?? ""))) {
  ...
  const assignmentByIdea = useMemo(() => {
    ...
    for (const a of assignments) {
      const list = m.get(a.idea_id) ?? [];
      list.push(a.category_id);
      m.set(a.idea_id, list);
    }
    return m;
  }, [assignments]);
  ...
  const counts = useMemo(() => {
    ...
    for (const cat of mainCategories) {
      ...
      for (const idea of ideas) {
        if ((assignmentByIdea.get(idea.id) ?? []).some((id) => id === cat.id || categoryById.get(id)?.parent_id === cat.id)) n++;
      }
      m.set(cat.id, n);
    }
    return m;
  ...
  const visibleIdeas = useMemo(() => {
    const f = selected === "all" ? [...ideas] : ideas.filter((idea) => (assignmentByIdea.get(idea.id) ?? []).some((id) => id === selected || categoryById.get(id)?.parent_id === selected));
  ...
  async function deleteIdea(idea: IdeaRow) {
    ...
    const ad = await s.from("idea_categories").delete().eq("idea_id", idea.id);
    if (ad.error) throw ad.error;
    const r = await s.from("ideas").delete().eq("id", idea.id).eq("user_id", u.id);
  ...
  async function deleteAllIdeas() {
    ...
    try {
      ...
      const { error: deleteError } = await s.from("ideas").delete().eq("user_id", u.id);
  ...
  return (
    <main className="mx-auto flex h-[100dvh] w-full max-w-[430px] flex-col overflow-hidden bg-[#fbfaf7] text-[#111]">
      ...
      {filterOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/25" onClick={() => setFilterOpen(false)}>
          <div className="w-full max-w-[430px] rounded-t-[28px] bg-[#fbfaf7] p-5 pb-8" onClick={(e) => e.stopPropagation()}>
            <div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-semibold">Filtern & sortieren</h2><button onClick={() => setFilterOpen(false)}><X className="h-5 w-5" /></button></div>
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-black/40">Kategorie</p>
            <div className="mb-5 flex flex-wrap gap-2">
              <button onClick={() => setSelected("all")} className={`rounded-full px-3 py-2 text-xs ${selected === "all" ? "bg-black text-white" : "bg-[#f0efeb]"}`}>Alle</button>
              {mainCategories.map((c) => <button key={c.id} onClick={() => setSelected(c.id)} className={`rounded-full px-3 py-2 text-xs ${selected === c.id ? "bg-black text-white" : "bg-[#f0efeb]"}`}>{c.name}</button>)}
            </div>
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-black/40">Sortierung</p>
            {([
              { id: "newest", label: "Neueste zuerst", icon: CalendarArrowDown },
              { id: "oldest", label: "Älteste zuerst", icon: CalendarArrowUp },
              { id: "az", label: "Titel A–Z", icon: ArrowDownAZ },
            ] as const).map((o) => {
              const Icon = o.icon;
              return <button key={o.id} onClick={() => setSort(o.id)} className="flex w-full items-center justify-between rounded-xl px-3 py-3 text-sm"><span className="flex items-center gap-3"><Icon className="h-4 w-4" />{o.label}</span>{sort === o.id && <Check className="h-4 w-4 text-[#79aa36]" />}</button>;
            })}
            <button onClick={() => setFilterOpen(false)} className="mt-4 w-full rounded-full bg-black py-3 text-sm font-semibold text-white">Anwenden</button>
          </div>
        </div>
      )}
    </main>
